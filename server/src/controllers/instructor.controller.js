...
const createQuestion = async (req, res) => {
  try {
    ...
    if (!examples.input || !examples.output || !examples.explanation) {
      return res.status(400).json({ message: "Invalid examples format", success: false });
    }

    // ✅ Ensure `sampleCode` has exactly 3 values (Java, C++, Python)
    if (!Array.isArray(sampleCode) || sampleCode.length !== 3) {
      return res.status(400).json({ message: "Invalid sample code format", success: false });
    }
    ...
    if (!exampleInDB) {
      return res.status(500).json({ message: "Failed to create example", success: false });
    }
    ...
    if (!sampleCodeInDB) {
      return res.status(500).json({ message: "Failed to create sample code", success: false });
    }
    ...
    if (!questionInDB) {
      return res.status(500).json({ message: "Failed to create question", success: false });
    }
